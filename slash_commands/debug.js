...
module.exports = {
		data: new SlashCommandBuilder()
				...
				.addSubcommand(subcommand =>
						subcommand
								.setName('tooltips')
								.setDescription(`Set a tooltip.`)
								.addBooleanOption(option =>
										option.setName('activated')
												.setDescription('Turn the small tooltips on (true) or off (false).')
												.setRequired(true)
								)
				),
		...
		async execute(bot, interaction, db) {
				switch (interaction.options.getSubcommand()) {
						...
						case 'tooltips': {
								db.set(`playersettings_${interaction.user.id}`, interaction.options.getBoolean('activated') ? '1' : '0')
								interaction.reply({ content: `Your tooltips have been turned ${interaction.options.getBoolean('activated') ? 'on' : 'off'}!`, ephemeral: true })
						}
				}
		}
}